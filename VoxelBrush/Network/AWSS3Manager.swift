...
class AWSS3Manager {
    ...
    private init () { }
    let bucketName = "voxelbrush"
    
    
    
    
    // Upload files like Text, Zip, etc from local path url
    func uploadWithUniqueName(fileUrl: URL, contentType: String, progress: progressBlock?, completion: completionBlock?) {
        let fileName = self.getUniqueFileName(fileUrl: fileUrl)
        self.uploadfile(fileUrl: fileUrl, fileName: fileName, contenType: contentType, progress: progress, completion: completion)
    }
